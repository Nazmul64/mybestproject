                    ...
                    <div class="form-group">
                        ...
                        @foreach ($getRecord as $getRecords)
                            <div class="row align-items-center" style="margin-bottom: 15px; background: #f8f9fa; padding: 10px; border-radius: 5px;">
                                <div class="col-md-4">
                                    <strong style="font-size: 16px; color: #333;">{{ $getRecords['name'] }}</strong> <!-- গ্রুপের নাম -->
                                </div>
                                <div class="col-md-8">
                                    <div class="row">
                                        @foreach ($getRecords['group'] as $group)
                                            <div class="col-md-3">
                                                <div class="custom-checkbox">
                                                    <input type="checkbox" name="permissions[]" value="{{ $group['id'] }}" id="permission_{{ $group['id'] }}">
                                                    <label for="permission_{{ $group['id'] }}" class="permission-label">{{ $group['name'] }}</label>
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
